...
disp(int2str(max_it));
while (it < max_it)    
    %Points is a vector that contains the indices for 4 random points in
    ...
    disp(['Computing the homography: ' int2str(it)]);
...
function idx_inliers = compute_inliers(H, x1, x2, th)
    ...
    d2 = geometricerror(x1,x2,H);% ToDo
    idx_inliers = find(d2 < th.^2);
    %idx_inliers = find(d2 < 10000000000);
end
